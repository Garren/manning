...
// create a unbuffered image channel fire off a goroutine to populate that
// channel. We return immediately which causes the channel to block, allowing
// the goroutine to start up. Once the goroutine completes it writes to the
// image channel
...
// create a new image, wait for each channel to get written to, then fire
// off a goroutine to copy the modified bytes from the channel into the new
// image at the appropriate coords. Encode the image as a jpeg, and finally
// encode the image as a base64 string and return
...
func mosaic(w http.ResponseWriter, r *http.Request) {
	...
	// fan-out. Quarter the image and process each piece concurrently
	...
	// fan-in. collect all of the processed pieces and combine
	// c is an unbuffered channel, so it will block until it gets written to
