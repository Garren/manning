<script>
  ...
  function deleteItem(item) {
    ...
    category = category; // trigger update
  }
  ...
  let dialog = null;
  let editing = false;
  let hovering = false; // set to t if we're currently dragging an item
  let itemName = "";
  let items = [];
  let message = "";
  ...
  <ul>
    {#each itemsToShow as item (item.id)}
      <div class="wrapper" animate:flip={options}>
        <!-- this bind causes the category object to update when the
            item packed value is toggled -->
        <Item
          bind:item
