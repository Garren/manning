<script>
  ...
  function onSelect(event) {
    ...
    selectedDogs = Array.from(selectedOptions).map(
      (option) => $dogStore[option.value] // the value of each option is a dog.id
    );
    dispatch("select", selectedDogs[0]); // remember the value of the selected dogs
  }
</script>
...
<div class="buttons">
  ...
  <!-- disabled if no dog is selected -->
  <button
